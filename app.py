...
try:
    ...
    latest = comp_daily.iloc[-1]
    
    # 핵심 지표 표시
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("최근 종가", f"{int(latest['종가']):,} 원")
    c2.metric("시가총액", f"{latest['시가총액']/1e8:,.1f} 억원")
    c3.metric("영업이익", f"{latest['영업이익']/1e8:,.1f} 억원")
    c4.metric("현재 POR", f"{latest['POR']:.2f} 배")
