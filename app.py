...
if not category_list:
  st.warning(
      "등록된 종목이 없습니다. 사이드바에서 카테고리나 종목을 추가해주세요."
  )
  st.stop()
...
selected_stock = st.sidebar.selectbox(
    "종목 선택:", list(available_stocks.keys())
)
...
for idx, yr in enumerate(past_years):
  ...
  with p_cols[idx]:
    ...
    if val_dart < 0:
      st.markdown(
          "<p style='color: #FF4B4B; font-size: 0.72rem; font-weight: bold;"
          " margin-top: -14px;'>🔴 적자</p>",
          unsafe_allow_html=True,
      )
      has_negative_op = True
    else:
      st.markdown(
          "<p style='color: #00C853; font-size: 0.72rem; margin-top:"
          " -14px;'>🟢 흑자</p>",
          unsafe_allow_html=True,
      )
...
with p_cols[5]:
  ...
  if input_2026 < 0:
    st.markdown(
        "<p style='color: #FF4B4B; font-size: 0.72rem; font-weight: bold;"
        " margin-top: -6px;'>🔴 적자 추정</p>",
        unsafe_allow_html=True,
    )
    has_negative_op = True
  else:
    st.markdown(
        "<p style='color: #00C853; font-size: 0.72rem; margin-top: -6px;'>🟢"
        " 흑자 추정</p>",
        unsafe_allow_html=True,
    )
...
if has_negative_op:
  st.warning(
      "⚠️ 영업이익이 적자(마이너스)인 구간은 POR 산출 공식상 'N/A' 처리되어"
      " 차트선이 연결되지 않을 수 있습니다."
  )
